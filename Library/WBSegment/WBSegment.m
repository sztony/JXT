...
- (id)initWithFrame:(CGRect)frame titleArray:(NSArray*)aTitleArray normalColor:(UIColor*)aNormalColor directionVertical:(BOOL)aIsVertical
{
    ...
    if (self) {
        ...
        titleArray =[[NSMutableArray alloc] initWithArray:aTitleArray];
        borderGap=(isVertical?width:height)/10;
        //selectedColor=[UIColor whiteColor];
        normalColor=[aNormalColor retain];
...
-(void)tap:(UITapGestureRecognizer *)tapGuesture
{
    ...
    [delegate WBVerticalSegment:self DidSelectedItemIndex:tapView.tag];
}
-(void)dealloc
{
    [titleArray release];
