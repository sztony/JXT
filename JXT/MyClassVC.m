...
-(NSInteger)numberOfSectionsInTableView:(UITableView *)tableView
{
    ...
    {
        if(segmentSelectIndexInitValue==2)
        {
            return 2;
        }
    }
...
-(UIView*)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if(section==0)
        headerLabel.text=@"我的老师";
    else
        headerLabel.text=@"我的同学";
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(tableView.tag==TAG_ContentTable)
    {
        ...
        {
             ClassMsgCell* cell=[tableView dequeueReusableCellWithIdentifier:@"ClassMsgCell_Setting"];
        ...
        {
            MemberListCell* cell=[tableView dequeueReusableCellWithIdentifier:@"MemberListCell_Setting"];
            NSDictionary* tmpDict=[self.contentDataArray objectAtIndex:indexPath.row];
...
-(void)loadTableContent
{
    ...
    [WBHTTPRequest sendRequestWithURL:url parameterDict:dict type:kWBHTTPRequestPOSTType  queue:[dataCenter globalTaskQueue] completeHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
        ...
        if(!connectionError)
        {
            ...
            {
                NSLog(@"class msg parse error:%@",error);
            }
