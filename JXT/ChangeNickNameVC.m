...
{
    [super viewDidLoad];
     field.text=[dataCenter userNickName];
    // Do any additional setup after loading the view.
}
-(IBAction)sureBtnClicked:(id)sender
{
    //URL
    NSURL* url=[NSURL URLWithString:[HOST_URL stringByAppendingString:URL_updateUserNickName]];
    //参数
    NSMutableDictionary* paraDict=[[NSMutableDictionary alloc] init];
    [paraDict setObject:[dataCenter userID] forKey:@"userId"];
    [paraDict setObject:field.text forKey:@"nickName"];
    
    //请求
    [WBHTTPRequest sendRequestWithURL:url parameterDict:paraDict type:kWBHTTPRequestPOSTType  queue:[dataCenter globalTaskQueue] completeHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
        ...
        if(!error)
        {
            ...
            if([msg hasPrefix:@"操作成功"])
            {
                dispatch_async(dispatch_get_main_queue(), ^{
                    NSLog(@"%@",msg);
                    alertLabel.text=msg;
                    [dataCenter setUserNickName:field.text];
                });
            }
        }
        else
